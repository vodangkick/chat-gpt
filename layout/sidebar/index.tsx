...
import { collection, deleteDoc, doc, where, query, orderBy,limit } from 'firebase/firestore';
...
function Sidebar({funcCloseMenu, handeShow, user} : Props) {
    ...
    const [chats, loading, error] = useCollection(
        query(userName && collection(db,'users', userName,"chats"),
    ...
    const handleOutSideSidebar = (e : any) => {
        if(!refMenuSiderBar.current.contains(e.target)) {
            funcCloseMenu();
        }
    }
    ...
    return (
        <>
            ...
            <div ref={refMenuSiderBar} className={`${styles.sidebar} ${handeShow && styles.openMenu}`}>
                <div className={`p-2 flex flex-col`}>
                    ...
                    <div ref={refMenuBottom} className={`${styles.logOut} text-gray-700 flex items-center`} >
                        ...
                        <ul  className={`${styles.menuBottom} ${showMenu === true && styles.openMenu}`} >
                            <li>
